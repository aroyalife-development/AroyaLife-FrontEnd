...
export default class Step1 extends Component {
	...
	isValidated() {
		...
		if (Object.keys(validateNewInput).every((k) => { return validateNewInput[k] === true })) {
			if (this.props.getStore().email !== userInput.email || this.props.getStore().gender !== userInput.gender || this.props.getStore().name !== userInput.name
				|| this.props.getStore().surname !== userInput.surname || this.props.getStore().mobile !== userInput.mobile || this.props.getStore().birthday !== userInput.birthday) { // only update store of something changed
	...
	_validationErrors(val) {
		const errMsgs = {
			...
			mobileValMsg: val.mobileVal ? '' : 'Mobile number should be 10 characters',
	...
	render() {
		...
		if (typeof this.state.birthdayVal === 'undefined' || this.state.birthdayVal) {
							...
							<form id="Form" className="form-horizontal mt-2">
								...
								<div className="form-group content form-block-holder">
									...
									<div>
										...
										/>
										<div className={notValidClasses.surnameValGrpCls}>{this.state.surnameValMsg}</div>
									</div>
								</div>
								<div className="form-group content form-block-holder">
									<label className="control-label">
								...
								<div className="form-group content form-block-holder">
									...
									<div>
										...
										/>
										<div className={notValidClasses.mobileValGrpCls}>{this.state.mobileValMsg}</div>
									</div>
								</div>
								<div className="form-group content form-block-holder">
									<label className="control-label">
									...
									<div>
										...
										/>
										<div className={notValidClasses.birthdayValGrpCls}>{this.state.birthdayValMsg}</div>
									</div>
								</div>
							</form>
						</div>
